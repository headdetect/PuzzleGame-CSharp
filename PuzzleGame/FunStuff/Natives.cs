...
namespace PuzzleGame {
    ...
    internal class Natives {
        ...
        public static void ExtendGlass( IntPtr handle, Margins bounds ) {
            ...
            try {
                Margins m = new Margins {
                    Bottom = Convert.ToInt32(bounds.Bottom * (DesktopDpiY / 96)),
                    Right = Convert.ToInt32 ( bounds.Right * ( DesktopDpiX / 96 ) ),
                    Left = Convert.ToInt32 ( bounds.Left * ( DesktopDpiX / 96 ) ),
                    Top = Convert.ToInt32 ( bounds.Top * ( DesktopDpiY / 96 ) ),
                };
                DwmExtendFrameIntoClientArea( handle, ref m );
            }
            catch {
                MessageBox.Show( "Vista/Win7 Found, but cannot load glass frames" );
            }
        }
